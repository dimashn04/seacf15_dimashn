...
import { addNewMovie, getAllMovies } from '../../api-helpers/api-helpers'
...
const AddMovie = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setNotification({
                open: true,
                message: 'Movie adding successful.',
                severity: 'success',
            })
        ...
        <div>
            <form onSubmit={handleSubmit}>
                <Box width={'50%'} padding={10} margin={'auto'} display={'flex'} flexDirection={'column'} boxShadow={'10px 10px 20px #ccc'}>
                    ...
                    <TextField error={isTitleWarning} helperText={isTitleWarning ? 'Movie title is already taken' : ''} value={inputs.title} onChange={handleChange} name='title' variant='standard' margin='normal' type='text' />
                    <FormLabel sx={labelProps}>
                        Description
                    </FormLabel>
                    <TextField error={isDescWarning} helperText={isDescWarning ? 'Invalid description' : ''} value={inputs.description} onChange={handleChange} name='description' variant='standard' margin='normal' type='text' />
                    <FormLabel sx={labelProps}>
                        Release date
                    </FormLabel>
                    <TextField error={isDateWarning} helperText={isDateWarning ? 'Invalid date' : ''} value={inputs.release_date} onChange={handleChange} name='release_date' variant='standard' margin='normal' type='date' />
                    <FormLabel sx={labelProps}>
                        Age Rating
                    </FormLabel>
                    <TextField value={inputs.age_rating} onChange={handleChange} name='age_rating' variant='standard' margin='normal' type='number' InputProps={{ inputProps: { min: 0 } }} />
                    <FormLabel sx={labelProps}>
                        Ticket Price
                    </FormLabel>
                    <TextField value={inputs.ticket_price} onChange={handleChange} name='ticket_price' variant='standard' margin='normal' type='number' InputProps={{ inputProps: { min: 0 } }} />
                    <FormLabel sx={labelProps}>
                        Poster URL
                    </FormLabel>
                    <TextField error={isPosterUrlWarning} helperText={isPosterUrlWarning ? 'Invalid poster URL' : ''} value={inputs.poster_url} onChange={handleChange} name='poster_url' variant='standard' margin='normal' type='text' />
                    <Box paddingTop={8}>
